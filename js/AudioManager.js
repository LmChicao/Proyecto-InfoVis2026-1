...
class AudioManager {
  ...
  init() {
    try {
      ...
      console.error('✗ Error al inicializar AudioContext:', e.message);
      this.isInitialized = false;
      return false;
    }
  }
  ...
  async loadAudio(fileName) {
    ...
    try {
      ...
      console.error(`✗ Error cargando ${fileName}:`, e.message);
      return null;
    }
  }
  ...
  async playSound(fileName, volume = 0.5) {
    ...
    try {
      ...
      // Reproducir
      source.start(0);
      console.log(`▶ Reproduciendo: ${fileName} (vol: ${volume})`);
    } catch (e) {
  ...
  async playAmbience(fileName, volume = 0.3, loop = true) {
    ...
    try {
      ...
      // Reproducir
      source.start(0);
      console.log(`▶ Ambience iniciada: ${fileName} (loop: ${loop})`);
      ...
      return {
        ...
        stop: () => {
          try {
            ...
            console.warn('Audio ya detenido');
          }
        }
      };
    } catch (e) {
      console.error('Error reproduciendo ambience:', e.message);
      return null;
    }
  }
    ...
    Object.keys(this.playingAudios).forEach(fileName => {
      if (this.playingAudios[fileName].isPlaying) {
        try {
          ...
          console.warn(`No se pudo detener ${fileName}`);
        }
      }
    });
